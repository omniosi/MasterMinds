...
function tumblrNotesInserted() {
    $(".more_notes_link").length ? Optica.LOADING_NOTES = !1 : Optica.DONE_LOADING_NOTES = !0
}! function(t, i) {
    var s = {
        init: function() {
            this.globals(), this.devices(), this.like_button(), this.in_iframe(), this.like_button(), this.link_color(), this.description_color(), t(".header-image").length && this.load_header(), Function("/*@cc_on return document.documentMode===10@*/")() && Optica.$body.addClass("ie10")
        },
        globals: function() {
            Optica.$win = t(window), Optica.$doc = t(document), Optica.$body = t("body"), Optica.$win_body = t("html, body")
        },
        in_iframe: function() {
            window.self !== window.top && Optica.$body.addClass("iframe")
        },
        ...
        load_header: function() {
            ...
            if (Optica.$body.hasClass("iframe") || Optica.$body.hasClass("touch")) i.css({
                opacity: 1
            }).addClass("loaded");
            else {
        ...
        link_color: function() {
            var t = Optica.Utils.hex_to_hsv(Optica.ACCENT_COLOR),
                i = Optica.Utils.hex_to_hsv(Optica.BACKGROUND_COLOR);
            t.s < .2 && t.v > .8 && (Optica.$body.addClass("light-accent"), i.s < .2 && i.v > .8 && Optica.$body.addClass("light-on-light")), t.v < .2 && i.v < .2 && Optica.$body.addClass("dark-on-dark")
        },
        description_color: function() {
            var i = t(".title-group .description");
            if (i.length) {
                var s = Optica.Utils.hex_to_rgb(Optica.TITLE_COLOR);
                i.css({
        ...
        devices: function() {
            ...
            this.is_touch_device() && (Optica.$body.addClass("touch"), Optica.$body.hasClass("permalink") || setTimeout(function() {
                window.scrollTo(0, 1)
            }, 1)), e.match(/(iPhone|iPod|iPad)/) ? this.ios() : (i = e.match(/Android\s([0-9\.]*)/)) && (s = i[1], Optica.$body.addClass("android"), parseFloat(s) < 4.4 && Optica.$body.addClass("android-lt-4-4")), t("#posts").on("click", "a.open-in-app", t.proxy(function(i) {
                i.preventDefault();
        ...
        ios: function() {
            Optica.$body.addClass("ios"), this.is_ios = !0
        },
        ...
        o = {
            ...
            event_loop: function() {
                Optica.$win.on("scroll.Eventor:next-frame", t.proxy(function() {
                    this.tick || (e(t.proxy(this.next_frame, this)), this.tick = !0)
                }, this))
            },
            next_frame: function() {
                Optica.$win.trigger("Eventor:scroll"), this.tick = !1
            }
        }, n = function(i, s) {
    ...
    n.prototype = {
        ...
        hide: function(i) {
            Optica.Popmenu.hide_all(), this.leave_delay = setTimeout(t.proxy(function() {
    ...
    var a = function(i, s) {
        this.$el = t(i), this.options = t.extend({
            ...
            inertia: .3,
            opacity_inertia: .4
        }, s || {}), this.inertia = this.options.inertia, Optica.$win.on("Eventor:scroll", t.proxy(this.__window_scroll, this))
    };
    a.prototype = {
    ...
    h.prototype = {
        ...
        _check_offset: function() {
            Math.abs(this.scroll_offset - Optica.$win.scrollTop()) > this.config.scroll_distance && (this.$search_input.is(":focus") || this.hide())
        },
        bind_events: function() {
            this.$el.on("touchstart click", this.trigger, this.events.trigger_click), Optica.$doc.on("click", this.events.document_click)
        },
        unbind_events: function() {
            this.$el.off("click", this.trigger, this.events.trigger_click), Optica.$doc.off("click", this.events.document_click)
        },
        destroy: function() {},
        show: function() {
            this.$glass && this.$glass.addClass("active"), this.$popover.parents("article").addClass("visible"), this.$popover.addClass("show"), this.$trigger.addClass("show"), setTimeout(t.proxy(function() {
                this.$trigger.addClass("active"), this.$container.addClass("active"), this.$popover.addClass("active"), this.scroll_offset = Optica.$win.scrollTop(), Optica.$win.on("Eventor:scroll", this.events.offset_scroll)
            }, this), 10)
        },
        hide: function(i) {
            ...
            Optica.$win.off("Eventor:scroll", this.events.offset_scroll), this.$glass && this.$glass.removeClass("active"), this.$container && this.$container.removeClass("active"), e.removeClass("active"), s.removeClass("active"), s.each(function() {
    ...
    var c = {
        __window_scroll: function() {
            Optica.DONE_LOADING_NOTES && this.unbind_events(), this._near_bottom() && !Optica.LOADING_NOTES && this.load_notes()
        },
        _near_bottom: function() {
            return Optica.$doc.height() - Optica.$win.scrollTop() < 3 * Optica.$win.height()
        },
        init: function() {
            Optica.LOADING_NOTES = !1, this.events = {
        ...
        load_notes: function() {
            Optica.LOADING_NOTES = !0, t(".more_notes_link").trigger("click")
        }
    }, l = function(i, s) {
            return this instanceof l ? (this.el = "string" == typeof i ? t(i).get(0) : i, this.$el = t(i), this.config = t.extend({}, l.defaults, s), this.config.$pagination || this.config.$pagination.length ? (this.current_page = this.config.$pagination.data("current-page"), this.next_page_number = this.current_page + 1, this.total_pages = this.config.$pagination.data("total-pages"), this.base_url = this.config.$pagination.attr("href"), this.base_url && (this.base_url = this.base_url.substring(0, this.base_url.lastIndexOf("/")) + "/"), this.loading_data = !1, this.is_scrolling = !1, this.body_timeout = -1, this.cache_selectors(), this.bind_events(), this.config.endless_scrolling && this.config.$pagination.length ? (this.config.$pagination.addClass("invisible"), this.init = !0) : this.init = !1, (Optica.$body.hasClass("touch") || this.$html.hasClass("lt-ie9")) && (Optica.GRID_LAYOUT = !1, this.is_grid_layout = !1, Optica.$body.removeClass("grid")), this.update_body(), this.updateMedia(), Optica.GRID_LAYOUT || this.update_spotify(), Optica.$body.hasClass("narrow") && this.upscale_images(), this.set_body_type(), l.register(this), this) : void 0) : new l(i, s)
        };
    l.prototype = {
        __document_keydown: function(i) {
            var s = i.charCode ? i.charCode : i.keyCode,
                e = i ? i.target : window.event.srcElement;
            t(e).is("input:focus") || this.is_grid_layout || (74 === s ? this.next_post() : 75 === s ? this.previous_post() : 190 === s && Optica.$win_body.animate({
                scrollTop: 0
            }))
        },
        ...
        __window_scroll: function() {
            Optica.$body.hasClass("touch") && this.update_body(), this.is_scrolling || (Optica.$body.addClass("is-scrolling"), this.is_scrolling = !0), clearTimeout(this.body_timeout), this.body_timeout = setTimeout(t.proxy(function() {
                Optica.$body.removeClass("is-scrolling"), this.is_scrolling = !1
        ...
        _near_bottom: function() {
            ...
            return Optica.$doc.height() - this.$el.scrollTop() < this.$el.height() * t
        },
        _near_top: function() {
            return !!(Optica.$win.scrollTop() < 50)
        },
        _slender: function() {
            return Optica.$win.width() < 720 ? !0 : !1
        },
        ...
        _append_new_posts: function(i) {
            ...
            }, this)), this.config.$target.append(s), this.updateMedia(s), Optica.GRID_LAYOUT && this.is_grid_layout ? e.imagesLoaded(t.proxy(function() {
                s.iframesLoaded({
            ...
            }, this)) : (this.loading_data = !1, this.update_spotify(s), Optica.$body.hasClass("narrow") && this.upscale_images(s), e.fadeTo(300, 1), this.hide_loader()), Tumblr.LikeButton.get_status_by_post_ids(o), window.ga && ga("send", "pageview", {
        ...
        set_body_type: function() {
            this._slender() ? (Optica.$body.addClass("slender").removeClass("grid"), Optica.GRID_LAYOUT && this.is_grid_layout && (this.config.$target.css({
                width: "auto"
            }), this.config.$target.masonry("destroy"), this.is_grid_layout = !1)) : (Optica.$body.removeClass("slender"), Optica.GRID_LAYOUT && Optica.$body.hasClass("index-page") && (Optica.$body.addClass("grid"), this.set_masonry()))
        },
        update_body: function() {
            this._near_top() ? (Optica.$body.addClass("top"), Optica.$body.removeClass("below-header")) : (Optica.$body.removeClass("top"), Optica.$body.addClass("below-header"))
        },
        ...
        bind_events: function() {
            this.$el.on("Eventor:scroll", t.proxy(this.__window_scroll, this)), this.$el.on("resize orientationchange", t.proxy(this._debounce(this.__window_resize, this.config.resizeDelay), this)), Optica.$doc.on("keydown", t.proxy(this.__document_keydown, this))
        },
...
}(jQuery, Optica), $(document).ready(function() {
    Optica.Utils.init(), Optica.Eventor.init();
    var t = $("body");
    t.hasClass("contain-header-image") ? new Optica.Parallaxer("body:not(.ie10, .touch) .parallax", {
        animate_opacity: !0,
        animate_position: !1
    }) : new Optica.Parallaxer("body:not(.ie10, .touch) .parallax"), new Optica.Pager(window, {
        $pagination: $("#pagination a"),
        $loader: $("#pagination .loader"),
        endless_scrolling: Optica.ENDLESS_SCROLLING
    }), $("body:not(.touch) #posts").drawer({
        trigger: "article:not(.exposed)"
    }), $("#page").popmenu(), t.hasClass("permalink") && $(".more_notes_link").length && Optica.ENDLESS_NOTES_SCROLLING && Optica.NotesPager.init()
});